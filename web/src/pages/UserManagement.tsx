...
/** Members and instructors only (excludes back-office admins — see Admins page). */
export default function UserManagement() {
  ...
  const [formRole, setFormRole] = useState<'member' | 'instructor'>('member');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const staff = allMembers.filter((m) => m.role !== 'admin');
      const starMap = new Map<string, number>();
      for (const e of leaderboard) {
        starMap.set(e.memberId, e.starCount);
      }
      const merged: MemberWithStars[] = staff.map((m) => ({
        ...m,
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (planFilter) rows = rows.filter((r) => (r.plan || 'free') === planFilter);
    return rows;
  ...
  function openEditModal(u: MemberWithStars) {
    ...
    setFormRole(u.role === 'instructor' ? 'instructor' : 'member');
      ...
      {loading ? (
        ...
        >
          <p style={{ color: colors.textMuted }}>
            {users.length === 0 ? 'No members or instructors yet.' : 'No users match the filters.'}
          </p>
        </div>
      ) : (
                ...
                <td style={{ padding: 12 }}>
                  <span
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 500,
                      backgroundColor: u.role === 'instructor' ? '#e0f2fe' : '#f1f5f9',
                      color: u.role === 'instructor' ? '#0369a1' : colors.textSecondary,
                    }}
                  >
                    {u.role}
                  </span>
